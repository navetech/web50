...
function showChannels(channels) {
    ...
    if (channels.length <= 0) {
        ...
        const template_item_none = template_channel_none;
        const id_elem_no_items = '#channels';
        addNoItemsInfo(item_show_hide, template_item_none, id_elem_no_items);
    }
...
function addNoItemsInfo(item_show_hide, template_item_none, id_elem_no_items) {
    // Generate HTML from template
    const context = {
        item_show_hide: item_show_hide
    }
    ...
    // Add HTML to page section
    document.querySelector(id_elem_no_items).innerHTML = content;
}
...
socket.on('remove channel', channel => {
    ...
    removeItem(id_elem_remove, id_elem_item_null, addNoItemsInfo);
});
